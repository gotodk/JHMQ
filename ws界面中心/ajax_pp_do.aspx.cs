        ...
        {/*
          ...
          */
            ...
            if (jkname == "处理结构代码文件生成")
            {
                ...
                //读取模板;
                string modstr_aspx = File.ReadAllText(Server.MapPath(ht_forUI["uimod"].ToString()).ToString());
                string modstr_cs = File.ReadAllText(Server.MapPath(ht_forUI["uimod"].ToString().Trim() + ".cs").ToString());
                //替换关键字
                ...
                string mkj = uinewfile.Substring(0, uinewfile.Length - 5 - 1).Replace("/", "_");
                ...
                {
                    //文件存在，提示
                    cljieguo = "界面文件路径已存在文件，不能写入！";
                }
                else
                {
                    //文件不存在，写入
                    File.WriteAllText(unf, modstr_aspx);
                    File.WriteAllText(unf + ".cs", modstr_cs);
                    cljieguo = "界面文件路径新文件成功从模板创建！";
                }
